...
if store.slot_item[my_id] = 5 {store.shotgun_frame+=1 store.wood+=1 store.metal+=4} //12gaugepump
if store.slot_item[my_id] = 6 {store.chemicals+=1 store.metal+=1} //Batteries
if store.slot_item[my_id] = 7 {store.metal+=2 store.wood +=1} //Backstabber
...
if store.slot_item[my_id] = 11 {store.shotgun_frame+=1 store.wood+=1 store.metal+=4} //Broken Shotgun
